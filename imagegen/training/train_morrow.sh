#!/usr/bin/env bash
# Morrow character LoRA on Z-Image Base (musubi-tuner, docs/zimage.md recipe).
# DiT: the Base transformer shards from the shared HF cache (first shard).
# VAE + Qwen3 TE: reused from ../z-image (identical for Turbo and Base).
set -euo pipefail
cd "$(dirname "$0")"
...
DATASET="./morrow_dataset.toml"
OUT="./output"
NAME="morrow_engraving_v1"

mkdir -p "$OUT" cache/morrow
